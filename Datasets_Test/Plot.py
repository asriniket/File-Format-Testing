...
def plot(file_formats, num_datasets, dimensions):
    if not os.path.exists("Data/Plots"):
        os.mkdir("Data/Plots")
    create_time, write_time, open_time, read_time, error = process_csv(
        file_formats, num_datasets, dimensions)
    width = .25
    ...
    plt.title(
        f"{num_datasets} Datasets {dimensions} Elements Dataset Read / Write Times")
    ...
    for i in range(0, len(file_formats)):
        ...
        bar_create_open = plt.bar(x + offset, [read_time_round, write_time_round], width,
                                  label=file_formats[i], edgecolor="black", yerr=[read_error, write_error], capsize=10)
    ...
    plt.ylabel("Average Time (ms)")
    plt.title(
        f"{num_datasets} Datasets {dimensions} Elements Dataset Create / Open Times")
    plt.xticks(x, plt_labels)
    for i in range(0, len(file_formats)):
        ...
        bar_create_open = plt.bar(x + offset, [create_time_round, open_time_round], width,
                                  label=file_formats[i], edgecolor="black", yerr=[create_error, open_error], capsize=10)
        plt.bar_label(bar_create_open, padding=3)
        offset += width
    plt.legend()
    plt.tight_layout()
...
def process_csv(file_formats, num_datasets, dimensions):
    ...
    for file_format in file_formats:
        ...
        dataset_create_time, dataset_write_time, dataset_open_time, dataset_read_time = df.iloc[:, 1:].mean(
            axis=0)
        create_deviation, write_deviation, open_deviation, read_deviation = df.iloc[:, 1:].std(
            axis=0)
        ...
        error.append([create_deviation, write_deviation,
                     open_deviation, read_deviation])
        ...
        df.to_csv(
            f"Data/{file_format}_{num_datasets}_{dimensions}.csv", index=False)
